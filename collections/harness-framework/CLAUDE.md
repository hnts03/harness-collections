...
- **Cost test**: 새 문서/스킬/에이전트 추가 전 "없으면 잘못 동작하는가?" 자문 (정식 정의: `harness-skill-template/references/skill-authoring-conventions.md` "Cost test (비용 자문)" 섹션)
- **Description 변경 = 재평가**: 기존 스킬·에이전트 description 수정 시 동일 PR에서 `/harness-benchmark` 실행 (정식 정의: `harness-skill-template/references/skill-authoring-conventions.md` "Description 변경 시 재평가 의무" 섹션)
- **유저 대상 보고 표현**: 사용자 직접 노출 텍스트(보고·AskUserQuestion·dev-plan)에서 마크다운 구현 디테일·메타정보 노출 금지. 의미 단위로 표현한다. (정식 정의: `harness-skill-template/references/skill-authoring-conventions.md` "유저 대상 보고 — 메타정보 노출 제한" 섹션)
...
- [2026-05-11] 플랜 011 완료: harness-skill-template.md References 분할 — Perplexity Progressive Disclosure 패턴 적용. 단일 439줄 reference 문서를 외부 진입점 1개(`harness-skill-template.md`, 79줄) + 4개 self-contained references(`harness-skill-template/references/{architecture-patterns, skill-authoring-conventions, session-runtime, pm-entry-visibility}.md`)로 분할. 응집 클러스터 단위 그룹화(architecture-patterns 5 H2 / skill-authoring-conventions 7 H3 묶음 단일 파일 / session-runtime 7 H2 / pm-entry-visibility H2+4 H3). 모드별 2개 참조 안내표 신설(`/harness` 모드 2건·`/pm` 모드 3건, skill-authoring-conventions는 양쪽 공유). 외부 11건 정식 정의 인용을 신규 path로 1:1 마이그레이션(CLAUDE.md 3건 + project-manager.md 1건 + create-skill 5건 + create-agent 2건), create-agent line 38은 sub-sub-section anchor("에이전트 description 축약 적용") 보존. researcher.md line 29 단일 파일 path 유지 + dispatch map 안내 한 문장 보강. project-manager.md line 319(reference_paths)·line 214(예시) 변경 없음. frontmatter version 0.4.0→0.5.0 bump. cross-reference 보정 2건(skill-authoring-conventions.md → session-runtime.md). stale linkage 1건(`references/agent-design-patterns.md`) 보존 — 별도 후속 플랜으로 이월. 플랜 009 후속 후보 "progressive disclosure (항목 7)" implicitly satisfy. 6개 결정 게이트(사용자 결정 2건: Option A entry shape·모드별 2개 안내표 / 연구 grounding 채택 4건: conventions 단일 파일·체크포인트 inline·skill-directory-rooted 경로·researcher single-path). 7 worker 병렬(worker F를 F1/F2로 분해 — scope 자가진단 룰 3파일 초과 회피). 플랜 흐름: 도메인 분석 → 미션 framing 3차 정정(skill 폴더화→공유 reference 분리→사용자 dispatch 위임 명료화) → 연구(1차 reviewer REWORK 3건: 외부 인용 카운트 8→11 정정·H2/H3 단위 표기·reference_paths 결정 위임 + PM 보정 → reviewer 2회차 OK 8/10) → 결정 게이트(6건) → 설계 OK 9/10 → 적용(7 worker 병렬, 20 AC) → 검증(qa PASS critical 0건, A·B·C·D 모두 합격, 4 호출 워크스루) → 문서화. 다음 플랜 후보 미해결 3건: stale linkage `references/agent-design-patterns.md` 처리, README Gotchas 시드, description 변경 자동화 훅 — 주요 산출물: harness-skill-template.md(재작성) + 4개 references + CLAUDE.md·project-manager.md·researcher.md·create-skill/SKILL.md·create-agent/SKILL.md 마이그레이션 + _workspace/011-template-references-split/{011-dev-plan.md, research-report.md, design-spec.md, qa-report.md} + docs/phase_011/011-session-report.md
