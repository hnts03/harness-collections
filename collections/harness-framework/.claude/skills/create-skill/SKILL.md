...
- **운영 절차와 정식 컨벤션 분리에 따른 컨벤션 누락**
  - 증상: 본 스킬의 운영 절차만 따르고 정식 컨벤션 참조를 따라가지 않아 트리거 매트릭스 작성 기준·description 3요소·Cost test 자문이 표면적으로 처리된다.
  - 발생 조건: 사용자가 본 스킬을 빠르게 호출하여 운영 절차를 그대로 따르되 참조 안내된 정식 컨벤션 영역을 로드하지 않을 때 발생한다.
  - 회피·완화: 본 스킬의 각 STEP과 운영 원칙 항목에서 명시된 참조 영역을 반드시 함께 로드한 뒤 본문 절차를 진행하고, 참조 미로드 상태에서는 새 스킬 생성을 보류한다.
  - 출처: 플랜 009 트리거 매트릭스 STEP 신설 기록.
