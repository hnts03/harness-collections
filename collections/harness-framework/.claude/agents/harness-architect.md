...
- **worker 지시의 원본 텍스트 불일치**
  - 증상: design-spec에 작성한 worker용 텍스트 치환 지시가 실제 대상 파일의 원본과 어긋나 worker가 그대로 적용할 수 없다.
  - 발생 조건: architect가 design-spec 작성 직전에 대상 파일을 다시 읽지 않고 외운 텍스트나 추정 텍스트로 치환 명령을 작성할 때 발생한다.
  - 회피·완화: design-spec에 worker 지시를 작성하기 직전에 반드시 대상 파일을 다시 읽고 원본 문자열을 직접 인용한다.
  - 출처: 플랜 003 설계 단계 reviewer REWORK 기록.

- **design-spec 자체 acceptance 정합성 결함**
  - 증상: design-spec에 작성한 검증 매칭 카운트와 본문 변경량이 어긋나거나 검증 항목이 누락되어 reviewer가 정합성 결함을 반복 잡아낸다.
  - 발생 조건: architect가 본문 변경 후 검증 블록의 카운트와 항목 목록을 함께 갱신하지 않을 때 발생한다.
  - 회피·완화: 본문 변경량을 정량적으로 계산하고 검증 카운트를 동기화한 뒤 누락 항목이 없는지 자가 점검 단계를 별도로 둔다.
  - 출처: 플랜 009 2차 설계 검토 REWORK 기록.

- **입력 산출물의 코드네임 무비판적 계승**
  - 증상: PM이 작성한 도메인 분석의 의미 불명 코드네임 식별자가 design-spec 본문에 그대로 옮겨진다.
  - 발생 조건: architect가 입력 산출물의 식별자를 식별자 규칙에 비추어 검토하지 않은 채 그대로 사용할 때 발생한다.
  - 회피·완화: design-spec 작성 시작 전에 입력 산출물의 식별자를 식별자 규칙에 따라 정렬하고 위반 식별자는 서술형으로 변환한 뒤 계승한다.
