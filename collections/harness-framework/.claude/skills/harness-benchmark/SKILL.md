...
- **description 변경 시 자가 트리거 누락**
  - 증상: 다른 자산의 description 필드를 수정했음에도 본 스킬을 자가 트리거하지 않아 description 변경 재평가 의무가 누락된다.
  - 발생 조건: 사용자나 PM이 description 변경을 단순 텍스트 수정으로 인식하거나, 자동화 훅이 없는 상태에서 변경 사실 자체를 의식적으로 추적하지 않을 때 발생한다.
  - 회피·완화: 동일 작업 단위 안에서 다른 자산의 description 필드를 변경했다면 본 스킬을 명시적으로 자가 트리거하고, 자동화 훅이 도입되기 전까지는 PM·사용자가 description 변경 여부를 작업 단계 완료 직전 자가 점검 항목으로 둔다.
  - 출처: 플랜 009 description 변경 재평가 의무 도입 기록.
