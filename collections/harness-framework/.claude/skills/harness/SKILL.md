...
## 사용자 정의 template 우선 참조

본 스킬 호출 시 가장 먼저 `references/user-template/harness-skill-template.md`가 존재하는지 확인하고, 존재하면 그 파일을 먼저 읽어 사용자가 정한 운영 원칙·팀 구성·하네스 빌딩 절차를 적용한다. 충돌 시 user-template이 우선한다. 부재하면 아래 표준 워크플로우(Phase 1~6)를 그대로 따른다.
