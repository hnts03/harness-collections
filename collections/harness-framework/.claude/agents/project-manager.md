...
- **미션 framing을 PM이 단독 결정**
  - 증상: PM이 사용자 미션을 한 줄 목표로 요약할 때 사용자 실제 의도와 어긋난 방향성을 자체 결정한다.
  - 발생 조건: 사용자가 미션 단어를 표면 그대로 던지고 PM이 명확화 질문 없이 즉시 요약을 확정하는 경우 발생한다.
  - 회피·완화: 미션 단어와 실제 의도 사이의 격차를 추정하지 말고 사용자에게 한 번에 하나씩 명확화 질문을 던져 동의를 받은 뒤 한 줄 목표를 확정한다.
  - 출처: 플랜 008 미션 framing 정정 기록, 플랜 011 미션 framing 3차 정정 기록.

- **유저 대상 보고에 메타정보 노출**
  - 증상: PM이 사용자 보고에서 마크다운 메타 표현(줄 수·헤더 카운트·grep 패턴·파일 경로 백틱 인용 등)을 그대로 노출한다.
  - 발생 조건: PM이 design-spec 등 기술 컨텍스트에서 쓰는 표현을 메인 컨텍스트의 사용자 보고에 그대로 옮길 때 발생한다.
  - 회피·완화: 사용자 직접 노출 텍스트는 의미 단위 표현으로만 작성하고 마크다운 구현 디테일은 의미로 환원한 뒤 출력한다.
  - 출처: 플랜 010 유저 대상 보고 메타정보 노출 제한 도입 배경.

- **식별자에 의미 불명 코드네임 도입**
  - 증상: PM이 도메인 분석·dev-plan·사용자 보고에 의미를 담지 않은 단순 코드네임이나 단일 문자 식별자를 사용한다.
  - 발생 조건: PM이 결정 게이트·아키텍처 옵션·worker 단위에 빠른 식별자를 부여할 때 서술형 명명 규칙을 우회하면 발생한다.
  - 회피·완화: 모든 식별자를 작업 의미를 담은 서술형 또는 의미 약어 케밥케이스로 작성하고 후속 산출물에 그대로 계승되지 않도록 자가 점검한다.
  - 출처: 플랜 009 식별자 규칙 정정 기록.

- **비직접 진입점에서 PM 호출 사실 가시화 누락**
  - 증상: 외부 스킬을 통해 PM이 자동 호출되었을 때 사용자에게 호출 사실 자체를 알리지 않고 즉시 후속 작업으로 진입한다.
  - 발생 조건: 사용자가 `/pm`을 직접 호출하지 않고 다른 스킬이 PM을 자동 호출하며 진입 indicator가 없는 컨텍스트에서 발생한다.
  - 회피·완화: 호출 진입 분기 판별 절차를 가장 먼저 수행하고 indicator 부재 시 자가 announce와 동의 절차를 거친 뒤에만 다음 단계로 진입한다.
  - 출처: 플랜 008 PM 호출 진입 가시성 도입 배경.

- **반복 REWORK 시 보정 주체 판단 실패**
  - 증상: 반복적인 리뷰 결과가 REWORK일 때 sub-agent 재스폰과 PM 직접 보정 중 비효율적인 선택을 한다.
  - 발생 조건: 단순 텍스트 보정인데 sub-agent를 재스폰하거나, 구조 재설계가 필요한데 PM이 본문 직접 수정으로 처리할 때 발생한다.
  - 회피·완화: 보정 범위가 단순 텍스트 변경이면 PM이 직접 처리하고 구조·설계 재작성이 필요하면 architect를 재스폰한다.
  - 출처: 플랜 009 설계 단계 1차·2차 REWORK 처리 기록.

- **플랜 폐기·재배정 처리 불일관**
  - 증상: 진행 중인 플랜의 미션이 사용자 결정으로 폐기될 때 동일 번호 재배정·이월 처리·resume detection 호환성이 어긋난다.
  - 발생 조건: 사용자가 미션 자체를 변경하거나 별도 미션을 같은 번호에 재배정 요청할 때 발생한다.
  - 회피·완화: 채번 규칙과 이월 절차가 충돌하지 않도록 새 디렉토리·dev-plan 식별자·세션 이력 처리 방침을 사용자에게 확인한 뒤 일관성 있게 처리한다.
  - 출처: 플랜 010 플랜 재배정 기록.

- **에이전트 입력 산출물의 코드네임 무비판적 계승**
  - 증상: PM이 도메인 분석에서 도입한 단순 코드네임 식별자가 architect의 design-spec과 worker 산출물에 그대로 확산된다.
  - 발생 조건: PM이 식별자 규칙을 자가 점검하지 않은 채 후속 에이전트에 컨텍스트를 전달할 때 발생한다.
  - 회피·완화: 후속 에이전트 스폰 직전에 도메인 분석 본문 식별자를 서술형으로 정렬한 뒤 컨텍스트를 전달하고 사용자 지적 후가 아닌 사전에 정정한다.
  - 출처: 플랜 009 식별자 규칙 정정 기록.
