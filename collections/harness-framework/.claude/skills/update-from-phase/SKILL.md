---
name: update-from-phase
description: Phase 완료 시 PM이 직접 트리거한다. dev-plan.md Phase 상태 갱신, CLAUDE.md 및 memory 갱신, git commit & push를 일괄 처리한다. "phase 완료", "update-from-phase", "phase 업데이트" 표현이 나오면 트리거한다. task 단위 갱신은 PM이 직접 수행하며 이 스킬은 덮어쓰지 않는다.
---

이 스킬은 **PM이 직접 트리거**한다. 필요한 컨텍스트:
- `<플랜번호>`: 현재 세션 플랜 번호 (3자리 숫자, 예: `007`)
- `<플랜 디렉토리>`: 현재 세션 플랜 디렉토리 이름 (`<NNN>` 또는 `<NNN>-<short-job-description>`, 예: `007-workspace-rename`)
- `<완료된 Phase 번호>`: 방금 완료된 Phase 번호
- `<다음 Phase 번호>`: 이제 시작될 Phase 번호 (마지막 Phase면 "없음")

---

## STEP 1 — dev-plan.md 갱신

`_workspace/<플랜 디렉토리>/<플랜번호>-dev-plan.md`를 읽어 다음을 수정한다:

- 완료된 Phase의 `**상태**`를 `완료`로 변경
- 다음 Phase의 `**상태**`를 `진행 중`으로 변경 (있으면)
- `## 현재 상태` 섹션 갱신:
  - **진행 중인 Phase**: <다음 Phase 또는 "완료">
  - **진행 중인 Task**: <다음 Phase의 첫 task 또는 "없음">
  - **다음 작업**: <설명>

PM이 task마다 갱신한 체크박스는 건드리지 않는다.

---

## STEP 2 — CLAUDE.md 갱신

`CLAUDE.md`의 `## 완료된 세션 이력` 섹션에 아래 형식으로 한 줄을 추가한다.

```
- [오늘 날짜] 플랜 <플랜번호> Phase <N>: <완료된 Phase 한 줄 요약> — 산출물: <주요 파일 또는 결정사항>
```

**마지막 Phase 완료 시** (세션 전체 종료): 날짜와 플랜번호, 이슈 목표 전체를 한 줄로 요약하여 추가한다.

```
- [오늘 날짜] 플랜 <플랜번호> 완료: <세션 목표 한 줄 요약> — 주요 산출물: <파일 목록>
```

CLAUDE.md가 없으면 이 단계를 건너뛴다.

---

## STEP 3 — memory 갱신

memory 시스템(`~/.claude/projects/.../memory/`)에 프로젝트 상태를 기록한다:
- 완료된 Phase 내용 요약
- 생성된 새 에이전트/스킬 정보
- 다음 Phase 계획

---

## STEP 4 — git commit & push

`/clean-commit` 스킬을 사용하여 커밋한다.

커밋 메시지 형식:
```
chore(phase-<N>): Phase <N> <Phase 이름> 완료

- <주요 산출물 1>
- <주요 산출물 2>
```

remote repo가 연결되어 있으면 push한다:
```bash
git remote -v 2>/dev/null | head -1
git push 2>/dev/null && echo "pushed" || echo "no remote or push failed (non-blocking)"
```

push 실패는 에러가 아니다. 로컬 커밋만으로 완료 처리한다.

---

## STEP 5 — 완료 보고

```
update-from-phase 완료
- Phase <N> → 완료 마킹
- Phase <N+1> → 진행 중으로 전환
- 커밋: <hash> <message>
- Push: 성공 / 스킵 (no remote)
```

## Gotchas

본 자산을 호출하거나 본문을 참조할 때 사전에 인지할 운영 함정을 정리한다. 정밀 분석이 필요하면 각 항목 끝의 출처를 따라간다.

- **phase 용어 사용 제한과 외부 호환성의 표기 비대칭**
  - 증상: 다른 자산은 "작업 단계" 표현으로 정렬되어 있는데 본 스킬의 이름과 본문에는 "Phase N" 표기가 남아 있어 사용자 발화·다른 자산과 표기 비대칭이 발생한다.
  - 발생 조건: 사용자가 "작업 단계 완료" 등 새 표현으로 발화하거나 PM이 본 스킬을 호출할 때 자산 간 표기 차이를 그대로 노출할 때 발생한다.
  - 회피·완화: 본 스킬의 이름과 description 트리거 키워드는 외부 호환성을 위해 유지하되, PM이 본 스킬을 트리거할 때 사용자 보고에서는 "작업 단계 완료 처리"로 의미 단위 표현을 사용하고 본 스킬 호출 자체는 식별자로만 다룬다.
  - 출처: 플랜 005·006 표기 정렬 결정 기록.
